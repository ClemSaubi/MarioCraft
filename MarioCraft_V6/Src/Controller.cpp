...
bool Controller::sourisSurElement(float x, float y, Element * e)const {
    ...
    if (x > e->getPosX() + DIMENSION_ARBRE)
        return false;

    if (y < e->getPosY())
        return false;

    if (y > e->getPosY() + DIMENSION_ARBRE)
            return false;
    ...
    if(_view->getWindow()->IsOpened())
    {
        ...
        while (_view->getWindow()->GetEvent(_event)) {
            ...
            if (_event.Type == Event::MouseButtonReleased and button == Mouse::Left)
            {
                ...
                {
                    Element * e = _model->searchElement(MousePos.x, MousePos.y);
            ...
            if (_event.Type == Event::MouseButtonReleased and button == Mouse::Right)
            {
                ...
                if (f != NULL)
                {
                    if (f->estActif() == true and sourisSurElement(MousePos.x, MousePos.y, f) == true)
                        _model->construireElement(f->getPosX(), f->getPosY() + DIMENSION_PERSO, "Artisan");
                }
                ...
                if (artisans.empty() == false)
                {
                    for (unsigned int i = 0; i < artisans.size(); ++i)
                    {
                        if (i%2 == 0)
                        {
                            artisans[i]->setDestX(MousePos.x + i*DIMENSION_PERSO);
                            artisans[i]->setDestY(MousePos.y);
                        }
                        else
                        {
                            artisans[i]->setDestX(MousePos.x + (i-1)*DIMENSION_PERSO);
                            artisans[i]->setDestY(MousePos.y + DIMENSION_PERSO  + 20);
                        }
                    }
                }          
            }
            ...
            if (_event.Type == Event::KeyReleased)
            {
                ...
                switch (_event.Key.Code)
                {
                        ...
                        if (_model->foyerConstruit() == false)
                        {
                            if (e != NULL)
                            {
                                if (sourisSurElement(MousePos.x, MousePos.y, e) == false)
                                    _model->construireElement(MousePos.x-(DIMENSION_SPRITE/2), MousePos.y-(DIMENSION_SPRITE/2), "Foyer");
                    ...
                    case Key::B : 
                    {
                        ...
                        {
                            if (sourisSurElement(MousePos.x, MousePos.y, e) == false)
                                _model->construireElement(MousePos.x-(DIMENSION_ARBRE/2), MousePos.y-(DIMENSION_ARBRE/2), "Bois");
                    ...
                    case Key::C :
                    {
                        ...
                        {
                            if (sourisSurElement(MousePos.x, MousePos.y, e) == false)
                                _model->construireElement(MousePos.x-(DIMENSION_SPRITE/2), MousePos.y-(DIMENSION_SPRITE/2), "Caserne");
                    ...
                    case Key::N : 
                    {
                        ...
                        {
                            if (sourisSurElement(MousePos.x, MousePos.y, e) == false)
                                _model->construireElement(MousePos.x-(DIMENSION_SPRITE/2), MousePos.y-(DIMENSION_SPRITE/2), "Nourriture");
                    ...
                    case Key::E : 
                    {
                        e = _model->searchElementActif();
                        Element * e2 = _model->searchElementCible();

                        if (e != NULL or e2 != NULL)
                            _model->eliminerElement(_model->indiceElement());
